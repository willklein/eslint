...
function writeFile(config, isJson, callback) {
    ...
    installDependencies(config, callback);
}

function installDependencies(config, callback) {
    var commands = [];

    if (config.plugins && config.plugins.indexOf("react") >= 0) {
        commands.push("npm i eslint-plugin-react --save-dev");
    }

    if (config.parser === "babel-eslint") {
        commands.push("npm i babel-eslint --save-dev");
    }
